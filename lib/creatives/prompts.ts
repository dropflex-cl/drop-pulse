...
import { CONCEPTS_PER_RUN, FAMILIES, FAMILY_DEFS, PROOF_GROUP, TEXT_ROLES } from "./catalog";
import { MAX_TEXTS, TEXT_LIMIT, type BakedText } from "./schemas";
...
export function creativesSystem(market: Market): string {
  return [
    ...
    "- El producto sale de la foto real (IMAGEN BASE): nunca describas su forma, color ni etiqueta; el sistema ya pide mantenerlo idéntico. En scene describe solo lo que lo rodea.",
    "- Un preset de Marketing Studio aporta la composición, la tipografía y el estilo de su grupo. Elige el que mejor calce con la familia, el producto y su paleta. Las familias native y letter van sin preset (preset_id null).",
    `- Pocos textos y cortos: el modelo escribe mejor 3 textos que 8, y en el feed nadie lee más. Máximo ${MAX_TEXTS} por concepto, cada uno ≤ ${TEXT_LIMIT} caracteres. Exactamente un headline de 2 a 7 palabras.`,
    ...
    "- scene va en inglés: fondo, props, luz, composición. Los props pueden ser libres y aspiracionales (frutas, agua, flores, texturas) si calzan con el producto; di cuáles.",
    "",
    "LAS 8 FAMILIAS",
...
export const QA_SYSTEM = [
  ...
  "- extra_texts: cualquier texto del anuncio que no se pidió (sobretítulos, firmas, botones, marcas, palabras sueltas), SIN contar lo impreso en la etiqueta del producto.",
...
export function qaUser(texts: BakedText[]): string {
