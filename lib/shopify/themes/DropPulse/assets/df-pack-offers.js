...
// - Y al revés: el pack elegido DENTRO del formulario de EasySell marca la misma tarjeta aquí.
//   EasySell vuelve a leer la cantidad de la ficha cada vez que abre su popup (también cuando lo
//   reabre detrás del downsell): sin esto, elegir 3 unidades en el formulario, cerrarlo y aceptar
//   el downsell lo devolvía al pack de la ficha. Se lee su store (solo lectura) después de su clic.
//
...
if (!customElements.get('df-pack-offers')) {
  ...
  /**
   * Cuántas unidades tiene la oferta elegida en el formulario de EasySell (getter
   * `offer/selectedOffer` de su store, el mismo que usa para cobrar). NaN si no se puede leer.
   */
  const easysellPackUnits = () => {
    const root = document.querySelector('#easysell .es-quantity-offers');
    const nodes = root ? [root, ...root.querySelectorAll(':scope > *, :scope > * > *')] : [];
    const store = nodes.find((el) => el.__vue__?.$store)?.__vue__.$store;
    return Number.parseInt(store?.getters?.['offer/selectedOffer']?.quantity, 10);
  };
  ...
  class DfPackOffers extends HTMLElement {
    connectedCallback() {
      ...
      document.addEventListener('click', this.onFormPack);
      this.apply();
      this.announce();
    }
    ...
    disconnectedCallback() {
      ...
      document.removeEventListener('click', this.onFormPack);
    }
    ...
    // Un pack elegido en el formulario de EasySell (clic o teclado, que también es un clic): después
    // de que la app termina su vuelta, se marca la tarjeta de esas unidades. Si no hay una tarjeta
    // visible con esas unidades, la ficha queda como estaba.
    onFormPack = (event) => {
      if (!this.usable || !(event.target instanceof Element) || !event.target.closest('#easysell .es-qo-container')) return;
      setTimeout(() => {
        const units = easysellPackUnits();
        const radio = this.radios.find((r) => Number.parseInt(r.value, 10) === units && !r.closest('.df-pack-offers__card')?.hidden);
