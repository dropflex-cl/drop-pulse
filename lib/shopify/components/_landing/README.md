...
- **Brillo en el botón de compra** (ajuste `df_easysell_motion`, encendido): cuando `#es-popup-button` entra en pantalla (60 % visible), una franja de luz lo cruza con un pulso de 1,03× en dos pasadas en unos 3 s, y se detiene. Así nada se mueve solo más de 5 s seguidos (WCAG 2.2.2). Vuelve si el botón sale y entra, hasta 3 veces por visita. No suena con `prefers-reduced-motion` ni si el comerciante activó la animación de EasySell (`cart-shaker`). Solo usa atributos `data-df-es-motion` / `data-df-es-shine`.
...
- **La primera vez, EasySell dibuja la tarjeta tarde:** pone `display: block` y la tarjeta del downsell (con sus botones) aparece en su siguiente vuelta, porque va con `v-if`. Por eso la capa vuelve a intentar con cada cambio del DOM mientras el popup siga abierto y sin decidir. Sin eso, en la tienda el primer cierre mostraba el popup de EasySell (visto en Datazo el 2026-09-26).
...
- **Montos:** EasySell descuenta el % sobre el **subtotal de productos** (menos el descuento por cantidad), no sobre el envío, el recargo ni los productos extra de un toque, y lo resta del total. El script lee esos valores de su store de Vue (solo lectura: `calculator/subtotal`, `calculator/offerDiscountValue`, `calculator/total`, `downsells/getDownsell`) y los formatea con `window.ES_FORMAT_CURRENCY`. Así el popup dice lo mismo que el resumen del formulario después de aceptar, incluido su redondeo: $55.990 al 5 % muestra total $53.191 y ahorro $2.799 (la resta de los totales que se ven).
- **En el resumen del formulario:** EasySell suma el descuento del downsell al que ya había (el del pack) y muestra una sola línea: al aceptar, «Ahorrado» pasaba de −$13.990 a −$16.090 sin un ítem propio. Con el downsell aceptado, el script deja en esa línea solo lo que no es downsell y agrega debajo «Descuento extra (5%) −$2.099» (ajuste `df_downsell_line`). Si no había otro descuento, la línea de EasySell se oculta y queda la propia. Es solo la vista: EasySell no lee esos nodos y el monto lo cobra su servidor.
- **Cifras que cuadran:** EasySell redondea cada monto por separado ($41.990 al 5 %: descuento $2.099,5 → «$2.100», total $39.890,5 → «$39.891»), y sus líneas no sumaban su total. El ahorro, en el popup y en el resumen, es la resta de los totales ya redondeados: $41.990 − $39.891 = $2.099.
...
- **Y al revés: el pack elegido en el formulario vuelve a la ficha.** EasySell lee la ficha cada vez que abre su popup, también cuando lo reabre detrás del downsell al cerrarlo. Elegir 3 unidades en el formulario, cerrarlo y aceptar el downsell lo devolvía al pack de la ficha. Ahora, después de un clic en una oferta del formulario (`.es-qo-container`), `df-pack-offers` lee las unidades elegidas en el store de EasySell (`offer/selectedOffer`, solo lectura) y marca esa tarjeta: la ficha, su precio y la cantidad del formulario quedan iguales. Sin una tarjeta visible con esas unidades, la ficha no cambia.
