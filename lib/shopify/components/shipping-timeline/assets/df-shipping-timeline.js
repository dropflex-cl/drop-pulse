...
if (!customElements.get('df-shipping-timeline')) {
  ...
  /** Día en que se prepara y despacha: sin feriados y, en modo hábil, sin sábado ni domingo. */
   ...
   * ship: order + días de preparación. from/to: ship + tránsito mínimo/máximo (días de entrega).
  ...
  const plan = (now, cal) => {
    ...
    return {
      ...
      order,
      ship,
      from: addDays(ship, cal.tmin, deliver),
      to: addDays(ship, Math.max(cal.tmin, cal.tmax), deliver),
  ...
  class DfShippingTimeline extends HTMLElement {
    ...
    render() {
      ...
      if (this.deliveryEl) this.deliveryEl.textContent = this.range(p.from, p.to, p.today);
      if (this.summaryEl) {
        const from = this.fmtLong.format(new Date(p.from * DAY));
        const to = this.fmtLong.format(new Date(p.to * DAY));
        const template = p.from === p.to ? t.summary_single : t.summary_range;
        this.summaryEl.textContent = (template || '').replaceAll('{from}', from).replaceAll('{to}', to);
      }
