...
const TOPICS = ["pago_cod", "envio", "uso", "talla_compat", "cuidado", "diferencial", "resultados", "garantia", "otro"] as const;
...
export const faqAndText = defineComponent({
      ...
      items: z
        .array(
          z.object({
            ...
            answer: noDigits("answer", POLICY_TOKENS).pipe(noHealthClaims("answer")).pipe(z.string().min(20).max(280))
              .describe("1ª frase responde directo («Sí.», «Entre {min} y {max} días hábiles.»), 2ª el detalle, 3ª opcional la red de seguridad."),
            topic: z.enum(TOPICS).describe("Tema, para cobertura y orden. No se muestra."),
          }),
        )
        .min(3)
        .max(8)
        .describe("Orden COD: pago_cod, envio, uso, talla_compat, diferencial, garantia al final."),
    })
    .superRefine((c, ctx) => {
  ...
  rules: [
    ...
    "Orden: pago_cod → envio → uso → talla_compat → diferencial → garantia (el cierre reduce el riesgo).",
  ...
  forbidden: [
    "Escribir números: plazos, días, costos, cantidad de clientes o pedidos (Ley 19.496 arts. 28 y 33).",
  ...
  examples: [
    ...
    },
    {
      ...
      heading_highlight: "necesitas saber",
      ...
      items: [
        ...
        { topic: "uso", question: "¿Cuánto rato al día lo uso?", answer: "Parte con ratos cortos y ve sumando tiempo. La idea es que te ayude a recordar la postura, no usarlo todo el día." },
