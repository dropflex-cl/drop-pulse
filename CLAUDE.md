...
- Pantallas: `/today`, `/products?filter=moving|stuck|published`, `/products/[id]` (`/base`, `/reviews`, `/angles`, `/copy`, `/images`), `/campaigns?period=today|7|30`, `/campaigns/[id]`, `/settings`. Onboarding: `/auth/create-account`, `/onboarding/shopify|products|numbers|meta|meta/accounts|done`. Integraciones: `/api/onboarding/*`, `/api/products/*`, `/api/webhooks/*`, `/api/cron/*`.
- **No hay etapa Precio.** El precio y los packs se definen en Información base (“Precio y packs”, requisito para optimizar); la ruta tiene 7 etapas: Información base, Reseñas (opcional), Ángulos, Textos (el comerciante la ve como “Página del producto”), Imágenes, Publicar y Anuncios (opcional). `/products/[id]/price` y `/productos/[id]/precio` redirigen a `/base`. `design-system/` todavía dibuja la etapa Precio y oferta (es la copia del artifact): en esto manda este archivo.
...
- **Todavía de ejemplo** (`lib/mock/`): imágenes generadas, campañas, supuestos y el asistente. `docs/esquema-supabase.md` es la propuesta original para esas partes (conceptos en español: al implementarlas, pásalas a inglés).
...
- **Etapa Textos: la página del producto** (`/products/[id]/copy`, `lib/copy/`, `lib/pipeline/copy.ts`, migración `20261001000000_page_copy.sql`, spec `docs/spec-textos.md`): se habilita con los 2 desarrollos aprobados. El redactor de página escribe los bloques de la página en la tienda (no el texto del anuncio: ese es el copywriter, en Anuncios). En la UI se llama “Página del producto” (`COPY_STAGE_TITLE`). Reglas:
