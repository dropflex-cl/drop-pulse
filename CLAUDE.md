...
- Pantallas: `/today`, `/products?filter=moving|stuck|published`, `/products/[id]` (`/base`, `/angles`, `/copy`, `/images`), `/campaigns?period=today|7|30`, `/campaigns/[id]`, `/settings`. Onboarding: `/auth/create-account`, `/onboarding/shopify|products|numbers|meta|meta/accounts|done`. Integraciones: `/api/onboarding/*`, `/api/products/*`, `/api/webhooks/*`, `/api/cron/*`.
- **No hay etapa Precio.** El precio y los packs se definen en Información base (“Precio y packs”, requisito para optimizar); la ruta tiene 6 etapas: Información base, Ángulos, Textos, Imágenes, Publicar y Anuncios. `/products/[id]/price` y `/productos/[id]/precio` redirigen a `/base`. `design-system/` todavía dibuja la etapa Precio y oferta (es la copia del artifact): en esto manda este archivo.
- Las claves internas siguen el vocabulario del design system (`StageKey` `angulos|textos|imagenes`, `ProductFilter` `detenidos…`); su traducción a URL vive en `lib/routes.ts` (`productHref`, `FILTER_PARAM`). No armes a mano una URL de etapa.
