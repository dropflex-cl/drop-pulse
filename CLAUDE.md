...
- **No hay etapa Precio.** El precio y los packs se definen en Información base (“Precio y packs”, requisito para optimizar); la ruta tiene 8 etapas: Información base, Reseñas (opcional), Ángulos, Textos (el comerciante la ve como “Página del producto”), Imágenes, Publicar, Creativos (opcional) y Anuncios (opcional). `/products/[id]/price` y `/productos/[id]/precio` redirigen a `/base`. `design-system/` todavía dibuja la etapa Precio y oferta (es la copia del artifact): en esto manda este archivo.
...
- **Reseñas (etapa opcional, `/products/[id]/reviews`)**: se importan de AliExpress con la lógica de dropflex v1 (`lib/reviews/aliexpress.ts` puro y testeado, `fetch.ts` con la red; endpoint `feedback.aliexpress.com/pc/searchEvaluation.do`, que no es un contrato). Migración `20260928000000_product_reviews.sql`. Reglas:
  ...
  - En la tienda se muestran siempre con la fuente visible (“Reseñas de compradores del mismo producto en AliExpress”). Publicarlas en Shopify todavía no existe: va con la etapa Publicar.
...
- **Etapa Página del producto** (`/products/[id]/copy`, `lib/copy/`, `lib/pipeline/copy.ts`, migraciones `20261001000000_page_copy.sql` y `20261008000000_page_components.sql`, spec `docs/spec-pagina-componentes.md`, diseño `design-system/textos.md`): se habilita con los 2 desarrollos aprobados. En la UI se llama “Página del producto” (`COPY_STAGE_TITLE`). Una sola llamada a Claude escribe la **ficha** (campos nativos: título, nombre corto, descripción corta, oferta, SEO; `lib/copy/listing.ts`) y el contenido de **cada componente de conversión** del catálogo (`lib/shopify/components`). Se guarda en `page_components` (uno vigente por componente). Reglas:
...
- **Etapa Imágenes** (`/products/[id]/images`, `lib/page-images/`, `lib/pipeline/page-images.ts`, migración `20261007000000_page_images.sql`, spec `docs/spec-imagenes.md`): las imágenes de la página del producto por espacio (Portada 1:1, Galería 1:1 de 4 a 6 ordenadas, un Beneficio 3:4 por cada beneficio de «Foto y razones» aprobado y en uso en la Página del producto). Se habilita con la ficha aprobada; queda lista con portada y 4 de galería.
