...
- Pantallas: `/today`, `/products?filter=moving|stuck|published`, `/products/[id]` (`/base`, `/reviews`, `/copy`, `/images`), `/campaigns?period=today|7|30`, `/campaigns/[id]`, `/settings`. Onboarding: `/auth/create-account`, `/onboarding/shopify|products|numbers|meta|meta/accounts|done`. Integraciones: `/api/onboarding/*`, `/api/products/*`, `/api/webhooks/*`, `/api/cron/*`.
- **No hay etapa Precio.** El precio y los packs se definen en Información base (“Precio y packs”, requisito para optimizar); la ruta tiene 6 etapas: Información base, Reseñas (opcional), Textos, Imágenes, Publicar y Anuncios (opcional). `/products/[id]/price` y `/productos/[id]/precio` redirigen a `/base`. `design-system/` todavía dibuja la etapa Precio y oferta (es la copia del artifact): en esto manda este archivo.
