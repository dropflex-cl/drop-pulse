...
- **Obligatorio: toda imagen entra a Storage optimizada** (`lib/media/optimize.ts`, con tests). Lo que puede ir a la landing (referencias por enlace o desde el equipo, fotos de reseñas, imágenes de la página generadas o subidas) se guarda con `optimizeImage`: WebP q86 `smartSubsample`, ≤ 2400 px por lado, EXIF aplicado, transparencia y animación conservadas. Las subidas con URL firmada se reemplazan al confirmar por su versión WebP y el original se borra. Lo que va a Meta (Creativos) usa `optimizeForAds` (JPEG mozjpeg q90), porque `/adimages` no garantiza WebP. `ensureImages` (`lib/shopify/publish/files.ts`) vuelve a optimizar antes de subir a Shopify Files: cubre lo guardado antes y las fotos que vienen de Shopify. Un paso nuevo que guarde imágenes pasa por aquí; nunca sube los bytes tal cual. Lo guardado antes se convierte con `npm run images:optimize` (simula; `-- --apply` convierte, `--user <id>`, `--limit <n>`): cambia la fila, borra la caché de `shopify_files` de ese archivo y después el original. `ad-media` no se toca.
...
- **Etapa Página del producto** (`/products/[id]/copy`, `lib/copy/`, `lib/pipeline/copy.ts`, migraciones `20261001000000_page_copy.sql`, `20261008000000_page_components.sql` y `20261016000000_copy_run_problems.sql`, spec `docs/spec-pagina-componentes.md`, diseño `design-system/textos.md`): se habilita con los 2 desarrollos aprobados y las imágenes listas (portada y 4 de galería; `CopyState.locked` dice qué falta). En la UI se llama “Página del producto” (`COPY_STAGE_TITLE`). Una sola llamada a Claude escribe la **ficha** (campos nativos: título, nombre corto, descripción corta, oferta, SEO; `lib/copy/listing.ts`) y el contenido de **cada componente de conversión** del catálogo (`lib/shopify/components`). Se guarda en `page_components` (uno vigente por componente). Reglas:
  ...
  - **GIFs** (`gif-strip`): la misma llamada escribe **5 textos** (título + párrafo o lista), el más fuerte primero. Los GIF no se eligen aquí: son los del espacio GIFs de Imágenes, en su orden, y el GIF N lleva el texto N (con 3 GIF, los 3 primeros textos). La vista previa los recibe en `StoreFacts.gifs`.
...
- **Etapa Imágenes** (`/products/[id]/images`, `lib/page-images/`, `lib/pipeline/page-images.ts`, migración `20261007000000_page_images.sql`, spec `docs/spec-imagenes.md`): las imágenes de la página del producto por espacio (Portada 1:1, Galería 1:1 de 4 a 6 ordenadas y 3 Beneficios 3:4 que propone el mismo director desde la ficha y los ángulos, `benefit-1…3`). Va **antes** de la Página del producto: se habilita con los 2 desarrollos de Ángulos aprobados y queda lista con portada y 4 de galería; cambiar los desarrollos la deja desactualizada. «Continuar: Página del producto» empieza a escribir la página.
  ...
  - **Espacio GIFs** (`gifs`, spec §1bis): hasta 5 subidos por el comerciante, ordenados, para `gif-strip`. Se re-codifican a WebP animado con sharp (`animated: true`, ancho ≤ 900 px); un archivo de un solo cuadro se rechaza. No entran al catálogo de fotos ni a la galería.
...
- **Eventos** (`/events`, `/events/[slug]`, `lib/events/`, `lib/pipeline/events.ts`, migración `20261015000000_events.sql`, spec `docs/spec-eventos.md`): capa de branding estacional (Cyber, Black Friday, Navidad…) sobre la página del producto. Nunca toca su contenido y se apaga sola. Reglas:
  - **Calendario:** vive en la tabla `events`, por mercado. Las fechas se editan sin deploy. El tema por defecto de cada tipo está en `EVENT_THEMES` (`lib/events/catalog.ts`, contraste AA probado en test).
  - **Activación:** el comerciante activa por tienda o por producto (`event_activations`). La del producto gana. Intensidad `subtle` | `medium` | `full` (`INTENSITY_LAYERS`). Toda la lógica está en `lib/events/resolve.ts` (puro, con tests).
  - **En la tienda:** el metafield de producto `dropflex.event` lleva los eventos próximos con su ventana. Liquid y el navegador deciden por hora, sin cron (`_event/`). El % de la etiqueta sale del tachado real y la cuenta regresiva va a la fecha real.
  - **Publicar:** se publica con el producto (`runPublish` → `publishProductEvent`) o solo el evento, desde «Publicar en la tienda» (segundo toque; cambia la tienda real).
  - **Textos del evento** (`event_copy`, paso `event_copy`): la IA reescribe solo la barra, la bajada y la etiqueta. **Avatar, ángulos y ficha son solo de lectura**: mantiene el ángulo principal y cambia el enfoque. Sin porcentajes ni montos fuera de PRECIO Y OFERTA. El comerciante aprueba.
