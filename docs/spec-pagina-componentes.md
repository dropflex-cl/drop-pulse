...
2. **Componentes**: los 15 de `lib/shopify/components/catalog.ts` (13 originales + `gif-strip`, los textos de los GIF, + `review-wall`, el muro de testimonios estilo Facebook), cada uno como **mini preview en React** fiel al Liquid, con el color del producto. Cada uno tiene un interruptor **«Usar en la página»**: lo marcado es lo que Publicar habilitará en la landing.
