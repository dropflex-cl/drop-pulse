...
import { montageFile } from "@/lib/video/catalog";
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string; scriptId: string }> }) {
  try {
    ...
    return new NextResponse(JSON.stringify(pkg, null, 2), {
      headers: { "Content-Type": "application/json; charset=utf-8", "Content-Disposition": `attachment; filename="${montageFile(pkg.angle.slot, pkg.format)}"`, "Cache-Control": "no-store" },
    });
  } catch (e) {
